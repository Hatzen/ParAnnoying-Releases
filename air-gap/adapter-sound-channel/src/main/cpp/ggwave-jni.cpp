...
// Adjusted by hatzen 29.07.2023
//

#include "ggwave-jni.h"
...
Java_de_hartz_software_parannoying_ggwave_SoundChannelHandler_initNative(JNIEnv * env, jobject obj) {
...
extern "C"
JNIEXPORT void JNICALL
Java_de_hartz_software_parannoying_ggwave_SoundChannelHandler_processCaptureData(JNIEnv *env, jobject thiz, jshortArray data) {
...
extern "C"
JNIEXPORT void JNICALL
Java_de_hartz_software_parannoying_ggwave_SoundChannelHandler_sendMessage(JNIEnv *env, jobject thiz, jstring message, jint options) {
    __android_log_print(ANDROID_LOG_DEBUG, "ggwave (native)", "Encoding message: '%s'", env->GetStringUTFChars(message, NULL));

    auto option = (ggwave_TxProtocolId) options;

    const int n = ggwave_encode(g_ggwave, env->GetStringUTFChars(message, NULL), env->GetStringLength(message), option, 10, NULL, 1);

    char waveform[n];

    const int ret = ggwave_encode(g_ggwave, env->GetStringUTFChars(message, NULL), env->GetStringLength(message), option, 10, waveform, 0);
