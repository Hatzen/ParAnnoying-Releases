...
class V0005ChannelSettings() : MigrationStep {
    override val REALM_VERSION_AFTER_MIGRATION = 6L
    ...
    override fun migrate(realm: DynamicRealm, oldVersion: Long) {
        ...
        val forwardPersistenceSchema = schema.get("SettingsPersistence")!!
            .addField("useWatermark", Boolean::class.java, FieldAttribute.REQUIRED)
            .setNullable("useWatermark", true)
        ...
        schema.get("FileMetaDataPersistence")!!
            .apply {
                addField("hmac_new", String::class.java)
                transform { obj ->
                    val intId = obj.getInt("hmac")
                    obj.setString("hmac_new", intId.toString())
                }
                removeField("hmac")
                renameField("hmac_new", "hmac")
            }
